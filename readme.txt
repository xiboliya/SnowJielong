...
代码仓库（皆采用git进行管理）：
GitHub：https://github.com/xiboliya/SnowJielong
Gitee：https://gitee.com/xiboliya/snowjielong
GitCode：https://gitcode.net/chenzhengfeng/snowjielong
...
4.编译项目
假设本项目的路径为：D:\SnowJielong，我们在命令行中cd到此路径下，然后输入如下命令：
ant
开始编译。编译完成后，当前目录下会产生一个ant目录。其中ant\bin目录存放编译产生的class文件和所需的资源文件（图片和配置文件等）。而ant\jar\SnowJielong.jar文件即是我们需要的可运行的jar包。此jar文件可以直接双击运行，或者直接双击本项目根目录下的SnowJielong.bat脚本文件运行，或者使用命令行：java -jar ant\jar\SnowJielong.jar来运行。

Linux或macOS系统：
1.下载并安装ant。
到官方主页http://ant.apache.org下载新版（撰写本文档时为Ant1.8.1）的ant，得到的是一个apache-ant-1.8.1-bin.zip的压缩包。将其解压到你的硬盘上，例如：~/apache-ant-1.8.1。其中~即为当前用户的home目录的简写。
...
4.编译项目
假设本项目的路径为：~/SnowJielong，我们在终端中cd到此路径下，然后输入如下命令：
ant
开始编译。编译完成后，当前目录下会产生一个ant目录。其中ant/bin目录存放编译产生的class文件和所需的资源文件（图片和配置文件等）。而ant/jar/SnowJielong.jar文件即是我们需要的可运行的jar包。此jar文件可以直接双击运行，或者直接双击本项目根目录下的SnowJielong.bat脚本文件运行，或者使用命令行：java -jar ant/jar/SnowJielong.jar来运行。
